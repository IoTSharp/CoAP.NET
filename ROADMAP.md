...
本文档只记录 CoAP.NET fork 自身的兼容、MVC Routing、性能和维护路线。CoAP 标准中的 Resource 概念继续保留在协议栈内部；新增 route / endpoint / controller 风格 API 的目的，是给应用代码一个更接近现代 .NET 服务器编程的入口，避免业务代码必须直接继承协议内部 Resource 类型。
...
- 新增 CoAP route adapter：`CoapRoute`、`CoapGet/Post/Put/Delete`、`CoapRouteContext` 和显式 route builder。
- 新增 CoAP MVC Routing：endpoint 数据源、route pattern、matcher、dispatcher、controller/action 描述、attribute route 和 convention route。
...
CoAP.NET 的主入口应与 ASP.NET Core MVC 的使用方式接近。宿主注册 CoAP server 和 MVC 服务，框架按宿主应用程序集自动发现 controller；业务项目只声明 controller/action，不再手写 `CoapRouteEndpoint.Create(...)`，也不直接 `new CoapServer()` 或传递 `IServiceProvider`。
...
`AddApplicationPart(...)` 只作为插件程序集、外部模块或测试场景的扩展入口，普通宿主业务 controller 不需要显式调用。
...
public sealed class SensorCoapController
{
    [CoapPost("readings")]
...
- resource registration：`AddCoapResources()` / `AddCoapMvc()` 注册 endpoint data source、controller discovery、matcher、dispatcher、binder、result executor 和 discovery provider。
...
- convention routing：按 controller/action 命名生成默认 route，允许宿主覆盖。
...
- DI 友好：controller 可由宿主服务容器创建，CoAP.NET 不持有业务单例。
...
- `CoapRouteContext` 已暴露 path、route values、queries、payload、Content-Format、Accept。
...
| C7 | `⬜` | controller attribute routing | 新增 `[CoapController]`、`[CoapRoute]`、method attributes、controller/action descriptor、application part 扫描；生成 endpoint 数据源。 |
| C8 | `⬜` | model binding 与 media negotiation | 支持 route value、query、request option、payload、`CancellationToken`、`CoapRouteContext` 参数绑定；补齐 Content-Format / Accept 匹配和 JSON binder 扩展点。 |
...
| C11 | `⬜` | Resource/MVC 示例与迁移文档 | 提供 `AddCoapServer()`、`AddCoapResources()`、`app.MapCoapResources()`、controller、JSON、binary payload、query option、Content-Format、Accept、Observe、发现输出和错误响应示例；说明 Resource 风格与 MVC 风格如何共存。 |
| C12 | `⬜` | 性能、AOT 与文档收口 | CoAP route benchmark、blockwise 大 payload 测试、DTLS PSK smoke、Observe smoke、trim/AOT warning 清单和发布检查清单。 |
| C13 | `⬜` | 宿主应用迁移落地 | 宿主应用移除手写 route endpoint 注册，改用 `AddCoapServer()`、`AddCoapResources()` 和 `app.MapCoapResources()`；保留自己的业务 controller、业务 DTO、授权、审计和领域服务调用。 |
        ...
        -> C4 routing core 抽象
          -> C5 AddCoapServer / AddCoapResources / app.MapCoapResources
            -> C6 dispatcher pipeline
              -> C7 controller attribute routing
...
- `ICoapEndpointDataSource`：暴露 endpoint 集合，供 route handler 和 controller 扫描共同使用。
...
目标是把宿主入口固定为常规 MVC 风格：服务注册发生在 `builder.Services`，controller 映射发生在 `app`，CoAP server 生命周期由 host 管理，让宿主应用不再显式创建 `CoapServer` 或 `CoapRouteEndpoint`。

- `AddCoapServer()`：注册 CoAP server 配置、监听端点、传输、DTLS、后台服务和生命周期。
- `AddCoapResources()`：按宿主应用程序集自动发现 CoAP controller/resource endpoint，注册 endpoint data source、matcher、dispatcher、binder、result executor、discovery provider。
- `AddCoapMvc()`：作为完整 MVC 能力入口或 `AddCoapResources()` 的兼容别名，包含 controller、filter、model binding、result executor。
...
- 普通业务 controller 不需要 `AddApplicationPart(...)`。
...
- `CoapActionInvoker`：调用 handler 或 controller action。
...
- handler 和 controller 共用同一套结果执行器。

### C7 controller attribute routing

目标是把业务项目从“手写 route 注册表”迁到“声明 controller/action”。

- controller 扫描：按 `[CoapController]` 或命名约定识别 controller。
...
- endpoint 生成：controller action 最终转成 `CoapEndpoint`。

验收：

- 示例 controller 可注册 `sensors/{sensor}/readings`、`diagnostics/{target}/status` 等通用路径。
- 业务项目不需要直接引用 `IResource`。
- controller route 与 handler route 可以共存。

### C8 model binding 与 media negotiation

目标是让 controller action 参数由 CoAP.NET 绑定，不让业务层手动拆协议细节。
...
- 特殊参数：`CoapRouteContext`、`CancellationToken`、远端 endpoint 信息。
...
- 支持 controller/action attribute 声明 discovery metadata。
- 与原 `DiscoveryResource` 共存，必要时由 discovery resource 查询 endpoint 数据源。

验收：

- `.well-known/core` 能列出 route/controller endpoint。
...
- 宿主应用可以把 access token、身份、领域对象校验挂到 filter 或 controller service，而不是写在 CoAP.NET。
...
| 业务路径 | 放到业务 controller attribute | 宿主应用 |
| access token、权限、租户、领域对象、消息发布、审计 | 业务 service、filter 或 controller action | 宿主应用 |
| 宿主领域语义 | 保持在宿主应用控制面 | 宿主应用 |

宿主应用第一轮目标：

1. 新增业务 CoAP controller，声明宿主自己的 resource 路径。
2. controller 内调用现有业务服务，先不重写业务逻辑。
3. 旧的 `CoAPService` 或手写 server 启动逻辑逐步退化为 CoAP.NET hosting 内部实现，宿主只通过 `AddCoapServer()` 配置端口和传输，通过 `app.MapCoapResources()` 映射业务 resource/controller。
4. 测试覆盖 controller 注册、匹配、业务调用和错误响应后，删除旧手写 route 注册。
5. 后续新增业务入口时继续增加 controller/action，而不是增加手写路由分发器。
...
- 宿主应用最终只保留业务 controller/action、业务 DTO、领域服务、权限和审计，不再拥有 CoAP routing framework。
- `.well-known/core` 的 route/controller 发现输出来自 CoAP.NET metadata。
